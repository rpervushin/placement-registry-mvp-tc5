...
1. **MVP** — учётка плейсментов RecSys: реестр, просмотр конфигурации, карта полок. Из внешних
   источников — только RecSys. Catalog не интегрируем, максимум — мониторинг запросов от него.
2. **Этап 2 — подключение новых источников информации о товарных полках** — интеграция с
   Каталогом (плейсменты как полноценный источник) и X5 Media.
3. **Конфигурационный слой и оркестратор (перспектива, не прорабатываем сейчас)** — реестр
   становится источником истины для конфигурации плейсмента (source, ranking_type, fallback),
   которую читают RecSys/Catalog при сборке выдачи. В сервис переходит управление приоритетами
   и правилами.
...
| Очереди/фон. задачи | `arq` (Redis-backed) | MVP | пересчёт discovery-событий, синхронизация с источниками |
...
- **RecSys** — единственная обязательная внешняя интеграция. У MS Catalog уже есть готовый
  ...
  вместо написания нового HTTP-клиента.
...
- **Catalog (полноценно)** — сервис уже содержит клиентские настройки на смежные системы
  ...
  у реестра появляется клиент к Catalog для синхронизации плейсментов и справочника
  товаров/категорий.
...
Только обязательная интеграция (RecSys), одна роль (Администратор реестра) и инфраструктурные
системы. Catalog — опциональный источник мониторинга, без обмена конфигурацией.
...
Добавляются роль «Владелец полки», X5 Media и Catalog как полноценные источники конфигурации,
плюс синхронизация с AB-платформой.
...
C4Context
  ...
  Rel(registry, recsys, "Передаёт placement_id + контекст запроса", "HTTPS/JSON")
  Rel(recsys, registry, "Возвращает ранжированную подборку", "HTTPS/JSON")
  Rel(registry, catalog, "Синхронизация плейсментов/категорий/товаров", "HTTPS/JSON")
...
C4Container
  ...
  System_Boundary(pr, "Placement Registry") {
    Container(adminui, "Admin UI", "React", "Учётка, карта плейсментов — MVP")
...
1. **C4-диаграммы** — готовы Context MVP (§8.1), Context Этап 2 (§8.2) и Container (§8.3) на
   основе реальной AS-IS схемы, сужены до контура реестра; при переносе в GitLab пересобрать в
   принятой в компании нотации (drawio/C2), если требуется единый формат с остальными арх.
   документами.
